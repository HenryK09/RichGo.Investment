...
###############################################
#multiindex
comp_name_df = pd.read_csv('C:/Users/user/PycharmProjects/fund_fee_Crawl/펀드판매회사.csv', encoding='cp949')
comp_name_lt = comp_name_df['koreanNm'].to_list()

comp_name_list = []
for comp_name in comp_name_lt:
    try:
        df = pd.read_csv(f'C:/Users/user/PycharmProjects/fund_fee_Crawl/판매사별_펀드_보수비용/{comp_name}.csv', encoding='cp949')
        print(f'{comp_name} data is here')
    except:
        print(f'no {comp_name} data')

    comp_name_list.append(df)

fund_df = pd.DataFrame(comp_name_list)

#fund_df = pd.concat([comp_name_list],df)
